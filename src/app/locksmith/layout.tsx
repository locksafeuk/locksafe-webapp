...
export default function LocksmithLayout({
  ...
  useEffect(() => {
    if (!isLoading) {
      ...
      } else if (isProtectedRoute && isAuthenticated && user?.type !== "locksmith") {
        // User is authenticated but not as a locksmith - redirect to appropriate portal
        if (user?.type === "admin") {
  ...
  }, [isLoading, isAuthenticated, isProtectedRoute, pathname, router, user?.type]);
