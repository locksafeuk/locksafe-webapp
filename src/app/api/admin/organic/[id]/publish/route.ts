...
export async function POST(
  ...
  try {
    ...
    if (publishToFB && !facebookAccount) {
      return NextResponse.json(
        { success: false, error: "Facebook account not configured. Please connect your Facebook Page in Settings." },
        { status: 400 }
      );
    }

    if (publishToIG && !instagramAccount) {
      return NextResponse.json(
        { success: false, error: "Instagram account not configured. Please connect your Instagram Business account in Settings." },
        { status: 400 }
      );
    }
